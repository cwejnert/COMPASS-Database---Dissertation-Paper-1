...
bullets("The confound","Better than it was, and still the binding constraint",
  ["THE ARMS ARE NOT DISJOINT BY MODEL, BUT ONE FAMILY DOMINATES ONE OF THEM. REMIND is 73% of High-RE and under 1% of High-CDR. MESSAGEix holds 99 High-CDR AND 27 High-RE, and seven of ten families hold both arms. Any pooled difference remains partly a modelling-framework contrast, and the improvement lowers the price of the claim rather than removing the need to pay it.",
   ...
   "THE WITHIN-MODEL EVIDENCE IS WHAT DECIDES. On jobs, the within-model comparison confirms the pooled direction in every family that holds both arms — the strongest within-model confirmation in the study, and the reason jobs is stated as a result rather than an association.",
   "ON DEPRIVATION THE WITHIN-MODEL MEDIAN OFTEN DISAGREES WITH THE POOLED DIRECTION. This is the outcome most exposed to which models populate which arm, and it is why the deprivation claim is stated as holding at high ambition and being regionally split rather than as a clean global finding — even though its World interval now clears."],
  {t:"THE CONFOUND",c:RED},
  "The within-model grid is built by W2_within_model.R. Note that it has not yet been re-run on the repaired keys and the all-CDR axis, so the within-model column is the one number in this deck still carrying the older sample — see 'What is open'.");

table("Is it real?","What survives, and what the reader should take on trust",
  ["","Pooled result","Holds on the engineered axis too?","Verdict"],
  [[{t:"Energy jobs",o:{bold:true}},{t:"20/20, all 20 significant",o:{bold:true,color:GREEN}},
    {t:"yes — 20/20, all significant",o:{color:GREEN}},{t:"a result",o:{bold:true,color:GREEN}}],
   [{t:"Energy deprivation",o:{bold:true}},"14/20, 11 significant, 2 against",
    {t:"yes — 15/20, 8 significant",o:{color:GREEN}},
    {t:"holds at 1.5°C; regionally split at 2°C",o:{color:GOLD}}],
   [{t:"PM2.5 mortality",o:{bold:true}},"8/20, 5 significant, 2 against",
    "yes — 9/20, 4 significant",
    {t:"Europe holds; World 1.5°C holds; Africa reverses",o:{color:GOLD}}]],
  [2.8,3.4,3.6,2.3],{t:"VERDICT",c:GOLD},
  "The asymmetry is the point. The same test applied identically to all three families clears jobs everywhere, qualifies deprivation by ambition level, and limits mortality to specific regions with one significant reversal. That cannot be dismissed as an unfair standard, and it is what makes the jobs result worth trusting.");
...
bullets("Limitations","Stated plainly, because they are load-bearing",
  ["MODEL COMPOSITION REMAINS THE BINDING CONSTRAINT. REMIND supplies 73% of High-RE against under 1% of High-CDR, and the all-CDR axis leaves seven of ten families holding both arms rather than eight. The within-model check supports jobs strongly and deprivation not at all, which is why the two are claimed with different force.",
...
bullets("What is open","Next steps, in priority order",
  ["RE-RUN THE MORTALITY TARGETS AGAINST THE ALL-CDR LABELS. The mortality sample was selected under the engineered axis, so 80 newly classified scenarios carry no mortality output. This is the one outstanding item that limits a headline result rather than a robustness check.",
   "RE-RUN THE WITHIN-MODEL GRID ON THE REPAIRED KEYS AND THE ALL-CDR AXIS. It is currently the only number in this deck computed on the older sample, and it is the number that decides how strongly each claim can be stated.",
