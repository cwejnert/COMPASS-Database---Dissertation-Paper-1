...
within<-pmap_dfr(designs,function(database,ambition_view){
 ...
 expand_grid(Region=ALLR,outcome=names(OUTS))%>%pmap_dfr(function(Region,outcome){
  ...
  f%>%filter(.data$Region==.env$rr)%>%group_by(fam)%>%summarise(n_cdr=sum(Pathway=='High-CMT'&!is.na(.data[[outcome]])),n_re=sum(Pathway=='High-RE'&!is.na(.data[[outcome]])),median_cdr=if(n_cdr>0)median(.data[[outcome]][Pathway=='High-CMT'],na.rm=TRUE)else NA_real_,median_re=if(n_re>0)median(.data[[outcome]][Pathway=='High-RE'],na.rm=TRUE)else NA_real_,raw_difference=median_re-median_cdr,comparable=n_cdr>=1&&n_re>=1,robust=n_cdr>=MINN&&n_re>=MINN,delta=if(comparable)sgn*cliff_d(.data[[outcome]][Pathway=='High-CMT'],.data[[outcome]][Pathway=='High-RE'])else NA_real_,.groups='drop')%>%filter(n_cdr>0|n_re>0)%>%mutate(database,ambition_view,Region,outcome,outcome_label=OUTS[outcome],direction_coded_difference=sgn*raw_difference,favours=case_when(delta>0~'High-RE',delta<0~'High-CDR',delta==0~'Tie',TRUE~NA_character_))
 })
...
summary<-within%>%filter(robust)%>%group_by(database,ambition_view,outcome_label)%>%summarise(cells=n_distinct(paste(Region)),family_cell_comparisons=n(),agree_pct=100*mean(agrees),within_favours_re_pct=100*mean(delta>0),.groups='drop')
