...
summary<-within%>%filter(eligible)%>%group_by(database,ambition_view,outcome_label)%>%summarise(cells=n_distinct(paste(Region)),family_cell_comparisons=n(),agree_pct=100*mean(agrees),within_favours_re_pct=100*mean(delta>0),.groups='drop')
